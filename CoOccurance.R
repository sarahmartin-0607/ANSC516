...
#setwd(to directory containing table.qza/metedata/

ASVs <- read_qza("table.qza")
ASV_table <- as.data.frame(ASVs$data)

#####################################################################
##This takes the table and lables each row by a shorter name
...
metadata<-read_q2metadata("projectmetadata.tsv")
str(metadata)

dataset <- merge(metadata, dataset, by.x = "SampleID", by.y = 0)
treatments<-as.vector(unique(dataset$diet)) 
datasetn<-dataset
datasetn[datasetn==0]<-NA
#one equal is an assignment, == means go into that and look for zeros
#i = 1

summary(metadata$diet)
#assign the n1-n! (however many samples ) based on what summary says, assign in order that the 
#summary presents. Maybe to half of each. # i choose 70% of all samples need to have this
my_column <- "diet"
n1 <- 12
n2 <- 16
n3 <- 16
...
for(i in 1:length(treatments)){
  ...
  # making an object that has all the results in it (both rho and P values): rcorr- draws all pairwise corralations
  ...
  #spearman is a ranked correlation
  #pearson correlation uses the actual values, not ranked data
  #make two separate objects for p-value and correlation coefficients
  rhos<-results$r #r squared values
  ps<-results$P
  ns<-resultsn$n #number of not zeros
...
write.csv(strong_results_taxa, "output/strong-results-taxa-diet.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="A"), "output/strong-results-taxa-A.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="B"), "output/strong-results-taxa-B.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="C"), "output/strong-results-taxa-C.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="D"), "output/strong-results-taxa-D.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="P"), "output/strong-results-taxa-P.csv", row.names = F)
